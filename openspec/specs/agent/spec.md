...
#### Scenario: Persist and compact session
- **WHEN** the agent processes conversation turns
- **THEN** it stores history in SQLite and triggers compaction every 3 turns.
...
#### Scenario: Log intent and outcome
- **WHEN** the agent performs an action
- **THEN** it generates a JSON log containing intent and outcome fields.
