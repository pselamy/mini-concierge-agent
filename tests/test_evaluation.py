...
class HITLUserSimulator(UserSimulator):
    ...
    async def get_next_user_message(self, events: list[Event]) -> NextUserMessage:
        ...
        if unanswered_call:
            ...
            return NextUserMessage(status=Status.SUCCESS, user_message=user_content)
                                ...
                                parts=[
                                    types.Part(
                                        function_call=types.FunctionCall(
                                            name="finish_task",
                                            args={
                                                "result": "Successfully booked a table at Gino's East for 7 PM."
                                            },
                                        )
                                    )
                                ],
...
async def test_automated_evaluation_suite(mock_client_class):
    ...
    eval_sets_manager.create_eval_set(
        app_name="mini-concierge", eval_set_id="golden_set"
    )
    eval_sets_manager.add_eval_case(
        app_name="mini-concierge", eval_set_id="golden_set", eval_case=eval_case
    )

    provider = CustomUserSimulatorProvider(queries=["Book Gino's East at 7 PM."])
